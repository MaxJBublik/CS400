/**
 * Some tests for MyList implementation of ListADT.
 */
import static org.junit.jupiter.api.Assertions.*;
import org.junit.*;
public class MyFirstJUnitTest {

    /** 
     * Checks the size of a new list, and also checks that size after adding,
     * and removing elements.
     * @returns true when this test passes, otherwise false
     */

    @Test
    public void testSizeOfList() {
	MyList<Boolean> list = new MyList<>();
	//if(list.size() != 0) {
	  //  System.out.println("Newly created list is not empty.");
	    //return false;
	//}
	assertEquals(0, list.size(), "Newly created list is not empty.");
	list.add(true);
	list.add(false);
	list.add(true);
	list.add(false);
	list.add(true);
	//if(list.size() != 5) {
	  //  System.out.println("List size is not 5 after calling add 5 times");
	    //return false;
	//}
	assertEquals(list.size(), 5, "List size is not 5 after calling add 5 times");

	list.remove(0);
	list.remove(1);
	list.remove(2);
	//if(list.size() != 2) {
	  //  System.out.println("List size is not 2 after removing 3 elements"
	//		       + " from a list of 5 elements");
	  //  return false;
	//}

	assertEquals(2, list.size(), "List size is not 2 after removing 3 elements from a list of 5 elements");
    }
    
    /**
     * By inserting 200 numbers this test should help ensure that the 
     * capacity of the underlying array is able to grow multiple times.
     * We then check that every pair of sequential elements in this list
     * are still in ascending order.
     * @returns true when this test passes, otherwise false
	*/    
    @Test
    public void testInserting200Numbers() {
	MyList<Integer> list = new MyList<>();
	boolean test;
	int current;
	int next;
	// fill list with ITERATIONS sequentail and increasing elements
	final int ITERATIONS = 200;
	for(int i=0;i<ITERATIONS;i++) {
	    list.add(i+1);
	}
	// ensure that elements are still in increasing order
	//for(int i=0;i<ITERATIONS-1;i++)
	    //if(list.get(i) >= list.get(i+1)) {
	//	String message = "Found non-ascending elements: "+list.get(i)+
	//	    ", "+list.get(i+1)+" at indexes "+i+", and "+(i+1);
	//	System.out.println(message);
	//	return false;
	for(int i=0;i<199;i++) { 
	     current = i;
	     next = i+1;
	     test = (list.get(current) >= list.get(next));
	     assertFalse(test, "Found non-ascending elements: " +list.get(current)+
		" , "+list.get(next)+" at indexes " +current+" and "+(next));
	}
    }

    /**
     * Removes an element from each end of the array, and then checks what is
     * left in the position of that removed element to test correctnesss.  This
     * method is also relying on and testing size() to compute last index.
     * @returns true when this test passes, otherwise false
     */ 
     @Test
     public void testRemovingFromEnds() {
	MyList<String> list = new MyList<>();
	list.add("apple");
	list.add("banana");
	list.add("cherry");
	list.add("durian");

	// test removal from the (highest index) end of the list
	String threeWas = list.remove(list.size()-1);
	String twoIs = list.get(list.size()-1);
	//if(!threeWas.equals("durian") || !twoIs.equals("cherry")) {
	String message = "Problem removing " + threeWas + " from the end" +
		"of the list, so that "+twoIs+" is the new final element.";
	  //  System.out.println(message);
	    //return false;
	String eq1 = "durian";
	String eq2 = "cherry";
	assertEquals(threeWas, eq1, message);
	assertEquals(twoIs, eq2, message);
    	// test removal from the (lowest index) start of the list
	String zeroWas = list.remove(0);
	String zeroIs = list.get(0);
	//if(!zeroWas.equals("apple") || !zeroIs.equals("banana")) {
	String newmessage = "Problem removing " + zeroWas + " from start" +
		"of the list, so that "+zeroIs+" is the new first element.";
	  //  System.out.println(message);
	    //return false;
	String eq3 = "apple";
	String eq4 = "banana";
	assertEquals(zeroWas, eq3, newmessage);
	assertEquals(zeroIs, eq4, newmessage);

	}

}   


